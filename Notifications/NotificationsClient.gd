...
func _play_queue():
	...
	while _queue.size() != 0:
		...
		notification_node.setup(
			_queue[0][0], _queue[0][1], _queue[0][2], _queue[0][3], _queue[0][4], _queue[0][5]
		)
		await notification_node.sized
		_queue.remove_at(0)
		custom_minimum_size.y = 0
		custom_minimum_size.y = notification_node.custom_minimum_size.y
