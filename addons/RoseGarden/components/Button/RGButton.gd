...
@onready
var texture: TextureRect = $HBoxContainer/VBoxContainer/MarginContainer/VBoxContainer/TextureRect
@onready var content_margin: MarginContainer = $HBoxContainer/VBoxContainer/MarginContainer

@export_category("Appearence")
@export_enum("Gray", "White", "Red", "Orange", "Yellow", "Green", "Teal", "Blue", "Pink", "Purple")
var color := "Gray":
	set(new_value):
...
@export var icon: Texture2D:
	set(new_value):
		icon = new_value
		_update()
@export_enum("None", "Left", "Right", "BothHorizontal", "Up", "Down", "BothVertical")
var connection := "None":
	set(new_value):
