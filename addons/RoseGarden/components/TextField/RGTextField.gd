...
@onready
var hint_text: Label = $MarginContainer2/VBoxContainer/HBoxContainer/NinePatchRect/MarginContainer/Label
@onready
var hint_texture: NinePatchRect = $MarginContainer2/VBoxContainer/HBoxContainer/NinePatchRect
...
func _on_gui_input(event: InputEvent) -> void:
	...
	if (event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_MASK_RIGHT and event.pressed):
		...
		menu.add_action(
			"Undo",
			load("res://addons/RoseGarden/icons/Undo.svg"),
			line_edit.menu_option,
			[line_edit.MENU_UNDO]
		)
		menu.add_action(
			"Redo",
			load("res://addons/RoseGarden/icons/Redo.svg"),
			line_edit.menu_option,
			[line_edit.MENU_REDO]
		)
