		...
		(
			...
			. tween_property(
				...
				(
					...
					* int(RoseGarden.Animations.rcmAppearance)
				)
			)
...
func _add_menu(data: Array):
	item_container.add_child(
		preload("res://addons/RoseGarden/components/RightClickMenu/RGrcm_item.tscn").instantiate()
	)
...
func _add_seperator():
	item_container.add_child(
		(
			preload("res://addons/RoseGarden/components/RightClickMenu/RGrcm_seperator.tscn")
			. instantiate()
		)
	)
	_update()
	...
	(
		...
		. tween_property(
			...
			(
				...
				* int(RoseGarden.Animations.rcmSelection)
			)
		)
...
func _on_mouse_entered() -> void:
	create_tween().tween_property(
		selection,
		"modulate",
		Color(selection.modulate.r, selection.modulate.g, selection.modulate.b, 1),
		(
			0.07
			* int(!RoseGarden.Accessibility.disableAnimations)
			* int(RoseGarden.Animations.rcmSelection)
...
func _on_mouse_exited() -> void:
	create_tween().tween_property(
		selection,
		"modulate",
		Color(selection.modulate.r, selection.modulate.g, selection.modulate.b, 0),
		(
			0.07
			* int(!RoseGarden.Accessibility.disableAnimations)
			* int(RoseGarden.Animations.rcmSelection)
...
func is_mouse_inside() -> bool:
	...
	if (
		...
		or mouse_pos.y < position.y
		or mouse_pos.x > position.x + size.x
		or mouse_pos.y > position.y + size.y
	):
		return false
	return true
