...
func update_shown_commands():
	...
	if command_list.size() == 0:
		...
		for i in range(command_container.get_child_count()):
			command_container.get_child(i).init(
				command_names[command_list[i]], command_icons[command_list[i]], command_list[i]
			)
	...
	if input_text == "":
		for i in range(commands.size()):
			if command_list.size() < command_amount:
				...
				command_points[command_list[command_list.size() - 1]] < command_points[commands[i]]
			):
		...
		for i in range(commands.size()):
			var score = 0.0
			var substring = float(
				_is_substring(command_names[commands[i]].to_lower(), input_text.to_lower())
			)
