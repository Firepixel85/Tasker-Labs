...
@onready
var option_handler: VBoxContainer = $VBoxContainer2/SceneContainer/MarginContainer/HBoxContainer/ScrollContainer/OptionHandler
@onready
var keybind_spacer: Control = $VBoxContainer2/SceneContainer/MarginContainer/HBoxContainer/KeybindSpacer
...
func _on_category_selected(category_id: String) -> void:
	...
	for option in Settings._option_order[category_id]:
		var option_node = option_handler.add_option(
			option, load(Settings.get_category(category_id)[option])
		)
