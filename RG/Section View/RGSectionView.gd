...
func add_section():
	_sections += 1
	section_container.add_child(preload("res://RG/Section View/RGsection.tscn").instantiate())
...
func _on_gui_input(event: InputEvent) -> void:
	var menu = RGmenu.new()
	menu.add_action("Previous",Icons.PREVIOUS,select_prev)
	menu.add_action("Next",Icons.NEXT,select_next)
