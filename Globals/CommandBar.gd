...
func link_action(action:Callable,path:String):
	if _client == null:
		Debug.warn("Process: "+Main.get_process_name(path.split("/")[0])+" attempted to link an action to a command while the client was not ready, action discarded. Command Path: "+path,ID)
		return ERR_BUSY
