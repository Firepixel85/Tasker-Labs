...
func add_option(category_id:String,option_id:String,option_scene:Resource,default_value):
	if !_settings_list.has(category_id):
		return ERR_DOES_NOT_EXIST
	if _settings_list[category_id].has(option_id):
		return ERR_ALREADY_EXISTS
	if !option_scene is PackedScene:
	...
	_settings_list[category_id][option_id] = option_scene
