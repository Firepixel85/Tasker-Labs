...
func get_plugin_name(plugin_id:String):
	...
	elif _developer_plugin == plugin_id:
		return JSON.parse_string(FileAccess.open("res://DeveloperPlugin/info.json",FileAccess.READ).get_as_text())["name"]
...
func get_plugin_version(plugin_id:String):
	if _plugins.has(plugin_id):
		...
		return JSON.parse_string(FileAccess.open("res://DeveloperPlugin/info.json",FileAccess.READ).get_as_text())["version"]
	return ERR_DOES_NOT_EXIST
...
func get_plugin_author(plugin_id:String):
	if _plugins.has(plugin_id):
		...
		return JSON.parse_string(FileAccess.open("res://DeveloperPlugin/info.json",FileAccess.READ).get_as_text())["author"]
	return ERR_DOES_NOT_EXIST
...
func get_plugin_target_version(plugin_id:String):
	if _plugins.has(plugin_id):
		...
		return JSON.parse_string(FileAccess.open("res://DeveloperPlugin/info.json",FileAccess.READ).get_as_text())["target_version"]
	return ERR_DOES_NOT_EXIST
...
func scan_available_plugins():
	...
	#Developer Plugin
	if FileAccess.file_exists("res://DeveloperPlugin/info.json") and Main.developerMode:
		var status = _verify_plugin("DeveloperPlugin","res://")
		if int(status) != ERR_INVALID_DATA:
			_developer_plugin = status
			Debug.log("Found developer plugin: "+JSON.parse_string(FileAccess.open("res://DeveloperPlugin/info.json",FileAccess.READ).get_as_text())["name"],ID)
	return OK
...
func load_plugin(plugin_id):
	if _plugins.has(plugin_id):
		...
		Debug.log("Loaded developer plugin: "+get_plugin_name(plugin_id),ID)
		load("res://DeveloperPlugin/plugin.gd").new().start()
		return OK
	else:
		return ERR_DOES_NOT_EXIST
